...
//GPS & navigation data
  ...
  uint16_t              week;                     // UTC Week number from GPS (weeks since Jan 1, 1980)
  uint32_t		time;	                  // UTC Time from GPS (1/100th sec since start of week)
...
//
#if defined(GPS_FILTER_VECTOR_LENGTH)
...
#if defined(GPS_LOGGER) && defined(GPS_LOG_IMU)
     // if an acc/att sensor update happened the write to it to the log     
     if(receivedCommands[0]>=(I2C_GPS_WP15+11))
       logIMU();
#endif
    }
}
...
#if !defined(GPS_LOGGER_NO_FILETIME) && defined(GPS_LOGGER)
void gps_time_callback(uint16_t* date, uint16_t* time)
{
  uint32_t secs = (i2c_dataset.time/1000) % 86400;
  *date = FAT_DATE(2012, 11, 15);
  *time = FAT_TIME(secs/3600, (secs/60)%60, secs%60);
}
#endif
...
#if defined(GPS_LOGGER)
  #if !defined(GPS_LOGGER_NO_FILETIME)
  SdFile::dateTimeCallback(gps_time_callback);
  #endif
  ...
  if (SD.begin(10)) {
    ...
    for(byte i=0; i<100; i++) {
      ...
      if(!SD.exists(filename)) {
        ...
        logEnabled = logFile; // true if file was opened
        break;
      }
    } 
  }
  if(logEnabled)
    bargraph_setled(STARTUP_SDCARD);
  else
    bargraph_displayerror(STARTUP_SDCARD);
#endif

  // setup done switch the bargraph
  reenable_satdisplay = millis()+3000;
}


#if defined(GPS_LOGGER)
#define FLUSH_LOG_EVERY 5000
void logGPS()
{
  if(!logEnabled) return;
  //digitalWrite(A1, HIGH);
  uint32_t tm = i2c_dataset.time + (millis()-lastframe_time);
  logFile.print('$');
  logFile.print(tm); 
  logFile.print(',');
  logFile.print(i2c_dataset.status.numsats);
  logFile.print(',');
  logFile.print(GPS_read[LAT]);
  logFile.print(',');
  logFile.print(GPS_read[LON]);
  logFile.print(',');
  logFile.print(i2c_dataset.altitude);
#if 0
#if defined(VENUS6) && defined(NMEA)
  logFile.print(',');
  logFile.print(i2c_dataset.ground_speed);
  logFile.print(',');
  logFile.print(i2c_dataset.ground_course);
#else
  logFile.print(',');
  logFile.print(venus_ctx.location.vel.x);
  logFile.print(',');
  logFile.print(venus_ctx.location.vel.y);
  logFile.print(',');
  logFile.print(venus_ctx.location.vel.z);
#endif
#endif
  logFile.println();
  //logFile.print(',');
  //logFile.println(i2c_dataset.baro);
  //digitalWrite(A1, LOW);

#if defined(FLUSH_LOG_EVERY)
  static uint32_t next_flush = 0;
  if(next_flush<millis()) {
    next_flush+=FLUSH_LOG_EVERY;
    bargraph_animate(BARGRAPH_KIT);
    suppressSatDisplay(350);
    logFile.flush();
  }
#endif
}

#if defined(GPS_LOG_IMU)
void logIMU()
{
  if(!logEnabled) return;
  uint32_t tm = i2c_dataset.time + (millis()-lastframe_time);
  ...
  logFile.print(tm); 
  logFile.print(',');
  logFile.print(i2c_dataset.acc[0]);
  logFile.print(',');
  logFile.print(i2c_dataset.acc[1]);
  logFile.print(',');
  logFile.print(i2c_dataset.acc[2]);
  logFile.print(',');
  logFile.print(i2c_dataset.attitude[0]);
  logFile.print(',');
  logFile.print(i2c_dataset.attitude[1]);
  logFile.print(',');
  logFile.println(i2c_dataset.heading);
}
#endif
...
bool GPS_VENUS_newFrame(uint8_t data)
{
  char result;
  if ((result=VenusProcessInput(data))==VENUS_OK) {
    //digitalWrite(A1, HIGH);
    if(venus_ctx.id==VENUS_GPS_LOCATION) {
#if 0 // if we did it as normal date/time
      double dow = venus_ctx.location.gps_tow/8640000;
      uint32_t tod = venus_ctx.location.gps_tow - dow;
      i2c_dataset.date = venus_ctx.location.gps_week*52 + venus_ctx.location.gps_tow/8640000;
      i2c_dataset.time = venus_ctx.location.gps_tow%8640000;
#else
      i2c_dataset.week = venus_ctx.location.gps_week;
      i2c_dataset.time = venus_ctx.location.gps_tow;
#endif
      i2c_dataset.status.gps2dfix = venus_ctx.location.fixmode>0;
      i2c_dataset.status.gps3dfix = venus_ctx.location.fixmode>1;
      i2c_dataset.status.numsats = venus_ctx.location.sv_count;
      i2c_dataset.altitude = venus_ctx.location.sealevel_alt/100;
      GPS_read[LAT] = venus_ctx.location.latitude;
      GPS_read[LON] = venus_ctx.location.longitude;

      // figure out the ground speed and course
#if 0
      uint32_t ground_speed = sqrt(venus_ctx.location.vel.x*venus_ctx.location.vel.x + venus_ctx.location.vel.y*venus_ctx.location.vel.y)/100;
      uint16_t course = _atan2(venus_ctx.location.vel.y, venus_ctx.location.vel.x);
      
      i2c_dataset.ground_speed = ground_speed;
      i2c_dataset.ground_course = course;
#else
      uint16_t course = _atan2(venus_ctx.location.vel.y, venus_ctx.location.vel.x);
      i2c_dataset.ground_speed = 0;
      //i2c_dataset.ground_course = 0;
#endif
      //i2c_dataset.ground_speed = course;
      //i2c_dataset.ground_course = course;
...
#endif
#if defined(VENUS6) && !defined(NMEA)
       if (GPS_VENUS_newFrame(Serial.read())) {
#endif
...
#if defined(BARGRAPH_DISPLAY)
  if(reenable_satdisplay==0 || reenable_satdisplay<millis()) {
    reenable_satdisplay=0;
    if(i2c_dataset.status.numsats == 0)
      bargraph_animate(BARGRAPH_LASER);
    else
      bargraph_set(BARGRAPH_UPTO, i2c_dataset.status.numsats);
  }
  bargraph_update();
#else
  // use a single LED to show status
  blink_update();   
#endif
